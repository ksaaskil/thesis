...
Modeling of energy transfer requires coupling at least some of the degrees of freedom to external reservoirs, which act as sources and sinks for energy. In this thesis, we employ Langevin baths as reservoirs. The coupling of an atomic chain to Langevin baths is illustrated in Fig. \ref{fig:langevin_chain}. In this case, the baths have two purposes: The baths colored in red and blue act as external heat sources and sinks, respectively, and are used to drive energy through the system. The baths colored in green, on the other hand, mimic internal fluctuations and dissipation arising from anharmonic scattering \cite{bolsterli70}, allowing for capturing non-linear effects in an effective manner \cite{bolsterli70}. 
...
In the linear approximation, the Langevin equations of motion for the three degrees of freedom $\bu_i$, $\bb{d}_i$, and $c_i$ are, respectively, \cite{bolsterli70,rosa10,rosa11,dhar03}
\begin{subequations}
\begin{align}
...
These equations contain, generally speaking, four different kinds of parameters: atom or dipole mass $m$, damping coefficients $\gamma$ and $\gamma_e$, Langevin noises $\xi_i$ and $\eta_i$, and coupling terms $K_{ij}$, $\bE_{ij}$ and $t_{ij}$ coupling the dynamics of different atoms, dipoles or orbitals, respectively. Formally, Eqs. \eqref{eq:th_eom1}, \eqref{eq:th_eom2} and \eqref{eq:th_eom3} are all connected through the Langevin physics describing fluctuations and dissipation, leading to similar form of equations and solution methods despite involving different energy carriers. 

While Eqs. \eqref{eq:th_eom1}, \eqref{eq:th_eom2} and \eqref{eq:th_eom3} are derived in more detail below in Sec. \ref{sec:th_eom2}, these equations can be easily understood intuitively. As an example, Eq. \eqref{eq:th_eom1} declares that the acceleration of atom $i$ is proportional to the sum of (i) the forces exerted by other atoms $j$, proportional for each atom pair $i,j$ to the force constant matrix $\bb{K}_{ij}$ defined below in Sec. \ref{sec:th_eom2}, (ii) stochastic Langevin force $\xi_i$ generating thermal fluctuations discussed in Sec. \ref{sec:th_langevin}, and (iii) Langevin friction $m_i \gamma \dot{\bu}_i$ responsible for dissipation. The assumption of a linear interparticle force is relaxed below in Sec. \ref{sec:th_eom2} to allow for more exact treatment of non-linear forces, necessary in the calculation of, e.g., frequency-dependent mean free paths. %The variance of the fluctuating force $\xi_i$ and the friction constant $\gamma$ are related by the fluctuation-dissipation theorem presented in Sec. \ref{sec:th_langevin}. 

Equation of motion \eqref{eq:th_eom2} for the dipole moment $\bb{d}_i$ is similar to Eq. \eqref{eq:th_eom1}, but here the force exerted by other dipoles $j$ is proportional to the electric fields $\bE_{ij}$, which is in turn linearly proportional to the dipole moment $\bp_j$. The background field $\Eenv$ is also included here for generality. The relation of this dipole equation of motion to the more commonly used fluctuating electrodynamics \cite{joulain05} is discussed in Sec. \ref{sec:th_eom2}. In the Heisenberg equation of motion \eqref{eq:th_eom3} for electron annihilation operator, on the other hand, the dynamics of different orbitals are coupled by the tight-binding hopping constant $t_{ij}$ allowing for electron hopping between orbitals. The equation for the creation operator $c_i^{\dagger}(t)$ is obtained by taking the Hermitian conjugate of Eq. \eqref{eq:th_eom3}. The presence of a single time derivative in the equation for ${c}_i(t)$ contrasts with the second time-derivatives of Eqs. \eqref{eq:th_eom1} and \eqref{eq:th_eom2} and is related to the fact that quantum-mechanical dynamics is mathematically described by diffusion in imaginary time \cite{}, giving rise to wave dynamics in real time.

\subsubsection{Solution of the linear Langevin equations}

Steady-state solution of Eqs. \eqref{eq:th_eom1}-\eqref{eq:th_eom3}, independent of initial conditions, can be found \cite{chaikin} by Fourier transforming the equations \footnote{We define Fourier transformation for any function $f(t)$, as usual, as \cite{chaikin}
\begin{equation}
...
\end{equation}}
Fourier transformation and expanding the electric field term $\bE_{ij}$ in Eq. \eqref{eq:th_eom2} as described in Sec. \ref{sec:th_eom2} leads to three linear systems of equations:
\begin{subequations}
\begin{align}
...
\end{align}
\end{subequations}
Here $\omega$ is the angular frequency. In Eq. \eqref{eq:th_eom_fourier_photon}, the electromagnetic Green's dyadic $\mathbb{G}^{\alpha\beta}(\br_i,\br_j;\omega)$ \cite{novotny} couples different dipoles. Each of the equations \eqref{eq:th_eom_fourier_phonon}, \eqref{eq:th_eom_fourier_photon}, and \eqref{eq:th_eom_fourier_electron} can be written in the matrix form
...
\subsubsection{Heat current to baths}
...
\subsubsection{Spectral analysis of interparticle heat current}
...
An exemplary computational setup for vibrational heat transfer modeling is illustrated in Fig. \ref{fig:chain_rg}(a) for an atomic chain. The atomic chain is considered for the simplicity of presentation, but the same setup is directly applicable to more complicated geometries such as the graphene constriction in Fig. \ref{fig:graphene_geom}, considered in \citepub{gf}. 

All atoms in the atomic chain of Fig. \ref{fig:chain_rg}(a) are coupled to Langevin baths responsible for thermal fluctuations and dissipation. One of the atoms is assumed to have a larger mass, thereby acting as a scattering center for phonons coming in from the left and right leads. Around this scattering center, bath temperatures are unknown, so they are determined self-consistently from the requirement that the heat current to each bath vanishes \cite{bolsterli70}. The requirement of zero heat current to local baths ensures that the heat current flowing in the chain is conserved at each atom site, a natural requirement in the steady-state. % The baths then act as quantum-mechanical temperature probes. 
...
The calculation of heat currents starts from writing down the Langevin equation of motion \eqref{eq:th_eom1} for each atom $i$. It is shown in \citepub{gf} that the solution of the equations for the leads and substitution to the equations for the center region allows for representing the leads by \textit{single} Langevin heat baths at temperatures $T_L$ and $T_R$, as illustrated in Fig. \ref{fig:chain_rg}(b). The microscopic details of lattice dynamics in the leads is then fully captured by the lead self-energy functions $\Sigma^L(\omega)$ and $\Sigma^R(\omega)$. Microscopic definitions of the self-energy function and the fluctuation-dissipation theorem for the lead baths are given in detail in \citepub{gf} and not presented here.
