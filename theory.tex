...
%<<<<<<< HEAD
%This section discusses the theory of lattice vibrations, electromagnetic field, and electron transport. The equations of motion of the three carriers are introduced in Sec. \ref{sec:th_eom}, starting from linearized Langevin equations of motion constituting a minimal (and often sufficient) model for energy transfer. In-depth discussion of the equations is  postponed to Sec. \ref{sec:th_eom2}. The Langevin terms describe both the creation and annihilation of carriers necessary in transport modeling. The properties of the Langevin baths for the three carriers are discussed in detail in Sec. \ref{sec:th_langevin}.  
% 
%=======
This section discusses the theory of lattice vibrations, electromagnetic field, and electron transport. The linearized equation of motion governing each heat transfer mechanism is presented in Sec. \ref{sec:th_eom}. Thermal fluctuations and dissipation are captured in the linearized equations by Langevin terms allowing for including both quantum effects and dissipation. Discussion of the general equations of motion is postponed to Sec. \ref{sec:th_eom2}. The properties of the Langevin baths for the three carriers are discussed in detail in Sec. \ref{sec:th_langevin}.  

% The presented equations of motion generally consist of deterministic terms derivable from microscopic Hamiltonians and Langevin force terms. The Langevin terms describe both the creation and annihilation of carriers necessary in transport modeling. 
...
%<<<<<<< HEAD
%This subsection summarizes stochastic equations of motion governing vibrational, electromagnetic and electronic energy transfer. These equations allow for fully quantum-mechanical derivation of energy transfer rates, account for wave effects and also capture dissipation at the level of one-particle relaxation rates. The presented equations for vibrational, electromagnetic and electronic energy transfer originate, respectively, from the works of Bolsterli, Rich and Visscher \cite{bolsterli70}, Rosa, Dalvit and Milonni \cite{rosa10,rosa11}, and Dhar and Roy \cite{dhar03}. We show how these seemingly unrelated equations allow for calculating energy transfer rates using very similar equations. The general equations, not limited by the linear approximation, are presented in Sec. \ref{sec:th_eom2}, where also the steps leading to the minimal equations are presented.
%=======
This subsection summarizes equations of motion governing vibrational, electromagnetic and electronic energy transfer. We introduce linearized stochastic equations of motion, which allow for fully quantum-mechanical derivation of energy transfer rates while accounting for wave effects and also for dissipation at the level of one-particle relaxation rates. These microscopic, Langevin equations of motion for vibrational, electromagnetic and electronic energy transfer originate, respectively, from the works of Bolsterli, Rich and Visscher \cite{bolsterli70}, Rosa, Dalvit and Milonni \cite{rosa10,rosa11}, and Dhar and Roy \cite{dhar03,dhar07}. All of these equations can be solved in a similar fashion, yielding frequency-resolved energy transfer rates in terms of Green's functions. These equations have not been presented side-by-side in earlier literature. Each of the equations is discussed in more detail in Sec. \ref{sec:th_eom2}, where also the steps leading to the linearized equations are presented and more references are given.
%>>>>>>> unified
\begin{figure}
 \begin{center}
 \end{center}
...
In the linear approximation, the Langevin equations of motion for the three degrees of freedom are $\bu_i$, $\bb{d}_i$, and $c_i$, respectively, \cite{bolsterli70,rosa10,rosa11,dhar03}
\begin{subequations}
\begin{align}
...
%<<<<<<< HEAD
%Equation of motion \eqref{eq:th_eom2} for the dipole moment $\bb{d}_i$ is similar to Eq. \eqref{eq:th_eom1}, but here the force exerted by other dipoles $j$ is proportional to the electric fields $\bE_{ij}$, which is in turn linearly proportional to the dipole moment $\bp_j$. The background field $\bE_0$ is also included here for generality. The relation of this dipole equation of motion to the more commonly used fluctuating electrodynamics \cite{} is discussed in Sec. \ref{sec:th_eom2}. In the Heisenberg equation of motion \eqref{eq:th_eom3} for electron annihilation operator, on the other hand, the dynamics of different orbitals are coupled by the tight-binding hopping constant $t_{ij}$ allowing for electron hopping between orbitals.
%=======
...
Finally, if the external field $\tilde{\bE}_0(\omega)$ appearing as a force term in Eq. \eqref{eq:th_eom2} is assumed to be a thermal field arising from the environment with dielectric constant $\epsenv(\br,\omega)$ and temperature $\Tenv$, the field satisfies the FDT \cite{novotny}
\begin{equation}
