...
This section discusses the theory of lattice vibrations, electromagnetic field, and electron transport. The equations of motion of the three carriers are presented in Sec. \ref{sec:th_eom}. For generality, we include both deterministic terms (derived from microscopic Hamiltonian) and external force terms arising from interaction with stochastic Langevin baths \cite{}. The stochastic fluctuations induced by the baths act as source terms in the equations, driving energy transfer \cite{}. Fluctuation-dissipation theorem requires the fluctuations to be accompanied by friction terms, damping carriers and including thereby dissipation. The properties of the Langevin baths for the three carriers are discussed in detail in Sec. \ref{sec:th_langevin}.  
...
In the case that the third-order term can be neglected, employing Eq. \eqref{eq:th_V_taylor} in the equation of motion \eqref{eq:th_eom} gives the system of linear equations, written here for each component $\alpha$:
\begin{equation}
...
Steady-state behaviour of the system, independent of initial conditions, can be analyzed by moving to frequency domain \cite{chaikin}. We define Fourier transformation of function $f(t)$, as usual, as
\begin{equation}
...
Microscopic dynamics of electrons is described by the quantum-mechanical Schr\"odinger equation \cite{ballentine}. Schr\"odinger's wavefunction formalism is, however, best suitable for a fixed number of particles. Because we want to model electron creation and annihilation arising from fluctuations and dissipation, respectively, the second-quantization formalism based on electron annihilation and creation operators \cite{ballentine} is more convenient. 

We use the tight-binding formalism \cite{datta} and assume, for simplicity, that each atom hosts only a single electron orbital. Excluding electron-electron and electron-phonon interactions, the tight-binding Hamiltonian for electrons can then be written as \cite{datta}
\begin{equation}
 \ca{H}_{\textrm{el}} = - \sum_{i,j} c_i^{\dagger} t_{ij} c_j.
\end{equation}
Here, electron creation operator $c_i^{\dagger}$ creates and annihilation operator $c_j$ annihilate electrons at electron orbitals localized at sites $i$ and $j$, respectively. The hopping constant $t_{ij}$ between electron orbitals at atoms $i$ and $j$ can be calculated from the Hamiltonian matrix element between the orbitals \cite{}. For graphene, the hopping constant between nearest-neighbor carbon atoms is approximately $2.7$ eV \cite{}.

The Heisenberg equation of motion $i\hbar \dot{c}_i = - \left[\ca{H}_{\textrm{el}},c_i \right]$ \cite{ballentine} accompanied by the stochastic Langevin force $\eta_i$ and Ohmic damping $-i\hbar \gamma_e c_i(t)$ \cite{dhar03} gives the equation of motion
\begin{equation}
...
To recap the linearized Langevin equations of motion, the equations for atomic displacements $\bu_i$, electron annihilation operators $c_i$, and dipole moments $\bd_i$ are:
\begin{subequations}
\begin{align}
...
\begin{equation}
 \langle \tilde \xi_i(\omega) \tilde \xi_i(\omega')^T \rangle = 4\pi \delta(\omega+\omega') k_B T \gamma \bb{I}_{3\times 3}. \label{eq:th_xixiom_ohmic_qm}
\end{equation}
