...
This chapter reviews the theory and methods used in this thesis. Sections \ref{sec:th_eom1} and \ref{sec:th_eom2} present the relevant equations governing phononic, electronic and photonic energy transfer in microscopic scale. To highlight the common features in the modeling of different carriers, we start by postulating the linearized equations for the three carriers in Sec. \ref{sec:th_eom1}. Carrier-specific details and more general equations are presented in \ref{sec:th_eom2}. 

Langevin theory is used throughout the thesis to model thermal fluctuations and dissipation, and this theory and the fluctuation-dissipation theorem are discussed in Sec. \ref{sec:th_langevin}. All calculations in the thesis aim at computing energy flow in the system, so the corresponding definitions and their evaluation are reviewed in Sec. \ref{sec:th_currents}. Section \ref{sec:th_currents} also presents the spectral analysis methods developed in Publications \cp{spectral} and \cp{cnt} for determining mode-wise contributions to interatomic energy transfer. The spectral contributions to energy current are given in terms of correlation functions, which can be determined for non-linear systems with classical molecular dynamics method, presented in Sec. \ref{sec:methods_md}.
...
\section{Linearized Langevin equations for phonon, photon and electron systems}
\label{sec:th_eom1}
...
This section reviews the general features of the linearized Langevin equations of motion governing energy transfer by lattice vibrations, electromagnetic field, and electrons. These equations account for both wave dynamics and dissipation in carrier propagation, allowing for simple but powerful modeling of energy transfer. The equations for vibrational, electromagnetic and electronic energy transfer originate, respectively, from the works of Bolsterli, Rich and Visscher \cite{bolsterli70}, Rosa, Dalvit and Milonni \cite{rosa10,rosa11}, and Dhar, Sriram Shastry and Sen \cite{dhar03,dhar06b}. These equations are applied in this thesis for quantum-mechanical Green's function calculations of vibrational energy transfer through a point contact (\citepub{gf}) and electromagnetic energy transfer between nanoparticles in a microcavity (\citepub{dipole}). The derivation of the linearized equations from more general equations and the precise definitions of parameters are presented in Sec. \ref{sec:th_eom2}.

Before writing down the equations of motion, we define the relevant degrees of freedom. Different energy transfer mechanisms and their corresponding degrees of freedom are schematically illustrated in Fig. \ref{fig:mechanisms}. Vibrational heat transfer in solids arises from the three-dimensional displacements $\bu_i$ of atoms from their equilibrium positions. An atom displaced from its equilibrium position exerts a force on other atoms, thereby leading to the propagation of displacement and transfer of energy. Similarly, electromagnetic energy transfer arises from fluctuations in local dipole moments $\bp_i=q\bd_i$, where $q$ is the dipole charge and $\bd_i$ the dipole displacement. An oscillating dipole radiates electromagnetic energy, scattered and absorbed by other dipoles. 

Quantum-mechanical electron transport in solids can be intuitively described by the tight-binding model \cite{}, in which electrons are imagined to move by hopping between electron orbitals localized at individual atoms. Mathematically, the dynamics of a single electron is governed by the one-particle Schr\"odinger equation \cite{griffiths_qm}, but to capture fluctuations and dissipation acting as electron creation and annihilation, it is more practical to formulate the dynamical equations for Fock operators $c_i^{\dagger}$ and $c_i$ \cite{ballentine} creating and and annihilating electrons at an orbital localized at atom $i$, respectively\footnote{Electron spin indices are suppressed throughout and each atom is assumed to host only a single atomic orbital.}. For notational simplicity, the same subindex $i$ is used for different degrees of freedom, but they are not necessarily related. 
...
Equations of motion for the three degrees of freedom $\bu_i$, $\bb{d}_i$, and $c_i$ are all connected through the Langevin physics describing fluctuations and dissipation, leading to a similar solution methods and phenomenology despite involving different energy carriers. In the linear approximation, to be discussed in more detail in Sec. \ref{sec:th_eom2}, the Langevin equations of motion are \cite{bolsterli70,rosa10,rosa11,dhar03}
\begin{subequations}
\begin{align}
 m_i \ddot{\bu}_i(t) &=  - \sum_j \bb{K}_{ij}\bb{u}_j(t) + \xi_i(t) - m_i \gamma \dot{\bu}_i, \label{eq:th_eom1} \\
 m_i \ddot{\bb{d}}_i(t) &= - m\omega_0^2 \bb{d}_i(t) + q\Eenv(\br_i,t)+q\sum_{j} \bb{E}_{ij}(t)+\xi_i(t) - m_i \gamma \dot{\bd}_i(t), \label{eq:th_eom2}\\
...
These equations contain, generally speaking, four different kinds of parameters: atom or dipole mass $m_i$, damping coefficients $\gamma$ and $\gamma_e$, Langevin noises $\xi_i$, $\eta_i$, and $\Eenv$, and coupling terms $\bb{K}_{ij}$, $\bE_{ij}$ and $t_{ij}$ coupling the dynamics of different atoms, dipoles or orbitals, respectively.  Different symbols $\xi_i$ and $\eta_i$ are used for Langevin noises, because they model fluctuations with different statistical distributions as detailed in Sec. \ref{sec:th_langevin}. The thermal background electric field $\Eenv(\br_i,t)$ is also included in Eq. \eqref{eq:th_eom2} for generality.

% Formally, Eqs. \eqref{eq:th_eom1}, \eqref{eq:th_eom2} and \eqref{eq:th_eom3}

Equations \eqref{eq:th_eom1}, \eqref{eq:th_eom2} and \eqref{eq:th_eom3} can be easily understood intuitively. As an example, Eq. \eqref{eq:th_eom1} declares that the acceleration of atom $i$ is proportional to the sum of (i) the forces exerted by other atoms $j$, proportional for each atom pair $i,j$ to the force constant matrix $\bb{K}_{ij}$ calculated from interatomic potential energy as discussed in Sec. \ref{sec:th_eom2}, (ii) stochastic Langevin force $\xi_i$ generating thermal fluctuations discussed in Sec. \ref{sec:th_langevin}, and (iii) Langevin friction $m_i \gamma \dot{\bu}_i$ responsible for energy dissipation at rate $\gamma$ \cite{li09jap}. The assumption of a linear interparticle force is relaxed below in Sec. \ref{sec:th_eom2} to allow for more exact treatment of non-linear forces, necessary in the calculation of, e.g., frequency-dependent mean free paths (\citepub{cnt}) and the thermal conductivity of twinning nanowires (\citepub{twinning}). %The variance of the fluctuating force $\xi_i$ and the friction constant $\gamma$ are related by the fluctuation-dissipation theorem presented in Sec. \ref{sec:th_langevin}. 
...
\end{equation}}.
Fourier transformation and rearrangement leads to three linear systems of equations:
\begin{subequations}
\begin{align}
...
\end{align}
\end{subequations}
Here $\omega$ is the angular frequency and the Greek indices $\alpha,\beta \in \{x,y,z\}$ stand for co-ordinate directions in Eqs. \eqref{eq:th_eom_fourier_phonon} and \eqref{eq:th_eom_fourier_photon}. Equation \eqref{eq:th_eom_fourier_electron} for electrons does not contain additional indices, as we assumed each atom $i$ to host only a single electron orbital. The electric field term $\bE_{ij}$ in Eq. \eqref{eq:th_eom2} has been expanded in terms of the the electromagnetic Green's dyadic $\mathbb{G}^{\alpha\beta}(\br_i,\br_j;\omega)$ \cite{novotny} as described in Sec. \ref{sec:th_eom2_photon}. % In Eq. \eqref{eq:th_eom_fourier_photon}, the electromagnetic Green's dyadic $\mathbb{G}^{\alpha\beta}(\br_i,\br_j;\omega)$ \cite{novotny} describes the electromagnetic coupling of dipoles and is defined in \ref{sec:th_eom2_photon}. 
...
\end{equation}
where $\bb{A}(\omega)$ is a coefficient matrix multiplying the degrees of freedom $\tilde{\bb{x}}(\omega)$ (standing for $\bu$, $\bb{c}$, or $\bb{d}$ written in vector form). The coefficient matrix captures the physical details of the system's dynamics. To arrive at the matrix form, the site indices and coordinates have been combined into a single composite index. 
...
With the fluctuation-dissipation theorem for the force variances $\langle \zeta^J(\omega) \zeta^J(\omega')^T \rangle$ presented in Sec. \ref{sec:th_langevin} and the solution \eqref{eq:th_gf_solution} available, one can calculate the thermal averages of any observable of interest. The calculation of vibrational heat current $ Q_I^{\textrm{bath}}$ to bath $I$, which could represent the locally dissipated power or the heat current flowing to external reservoirs, is outlined in Sec. \ref{sec:th_currents} and carried out in detail in \citepub{gf}. The final result is the Landauer-B\"uttiker-like expression \cite{landauer57,buttiker92} % (see, e.g., Refs. \cite{dhar03} and \cite{dhar06})
\begin{equation}
...
The general equations of motion governing lattice dynamics are dictated by the lattice Hamiltonian \cite{ziman}
\begin{equation}
 \ca{H}_{\textrm{ph}} = \sum_{i=1}^N \frac{(\bp^{\textrm{kin}}_i)^2}{2m_i} + \ca{V}(\bb{r}_1,\dots,\bb{r}_N). \label{eq:th_hamiltonian}
\end{equation}
Here $\br_i$, $\bp^{\textrm{kin}}_i$, and $m_i$ are the position, kinetic momentum and mass of atom $i$, respectively\footnote{The notation $\bp_i$ is reserved for dipole moments.}. The total number of atoms (which can also be infinite) is denoted by $N$. The first term of Eq. \eqref{eq:th_hamiltonian} is the total kinetic energy of the atoms and the second term $\ca{V}$ is the interatomic potential energy responsible for the interatomic interactions. The choice of the potential energy function $\ca{V}$ is crucial for an accurate description of the lattice dynamics and, consequently, of energy transfer. Models for $\ca{V}$ used in this work are presented in Publications \cp{fpu},\cp{fpu2}, \cp{spectral}, \cp{cnt}, and \cp{twinning} and in conjunction with the corresponding results in Sec. \ref{sec:results}.
...
\end{equation}
As explained in Sec. \ref{sec:th_eom}, the Langevin terms $\xi_i$ and $m_i\gamma \dot{\bb{r}}_i$ generally have two roles in the modeling of heat transfer. They are used both for (i) coupling atoms located at system's boundaries to external heat sources and sinks,  and (ii) to describe dissipative processes in the center region, if non-linear interactions accounting for phonon-phonon interactions are neglected (see below). The form of the friction term $m_i \gamma \dot{\bb{r}}_i(t)$, proportional only to instantaneous velocity, is called Ohmic due to its analogoue with a resistor in an electrical circuit \cite{weiss}. In general, the friction term is a time convolution giving rise to frequency-dependent damping \cite{weiss}, but in all publications included in this thesis, only the Ohmic form is used due to its simplicity.
...
\begin{equation}
 \ca{V} = \ca{V}_0 + \frac{1}{2} \sum_{i,j} \sum_{\alpha,\beta} u_i^{\alpha} K_{ij}^{\alpha \beta} u_j^{\beta}  + \frac{1}{6} \sum_{i,j,k} \sum_{\alpha,\beta,\gamma} \left. \frac{\partial^3 \ca{V}}{\partial u_i^{\alpha} \partial u_j^{\beta} \partial u_k^{\gamma}} \right|_{\bu=\mathbf{0}} u_i^{\alpha} u_j^{\beta} u_k^{\gamma} + \mathcal{O}(u^4). \label{eq:th_V_taylor} % B_{ijk}^{\alpha\beta\gamma}
\end{equation}
...
\end{equation}
The third and fourth term on the right-hand side of Eq. \eqref{eq:th_V_taylor} are the anharmonic terms that account for phonon-phonon interactions. First-principles calculation of thermal conductivities and phonon mean free paths requires explicitly including these terms in the equations of motion. The non-linearity of the resulting equations generally prevents their direct solution, so one needs to turn either to computationally heavy Keldysh Green's function techniques \cite{wang08} or simulating lattice dynamics using the classical molecular dynamics (MD) method \cite{allentildesley}. MD is used in Publications \cp{fpu}, \cp{fpu2}, \cp{twinning}, \cp{spectral}, and \cp{cnt} due to its flexibility and applicability to large system sizes and is reviewed in Sec. \ref{sec:methods_md}.
...
At sufficiently low temperatures or when dissipative events are modeled by local Langevin baths, anharmonic terms can be neglected. Including only the two first terms in the right-hand side of Eq. \eqref{eq:th_V_taylor} in the equation of motion \eqref{eq:th_eom} gives the system of linear equations \eqref{eq:th_eom1}. The linearity of the equations allows for including quantum statistics through the quantum fluctuation-dissipation theorem for $\xi_i$, discussed in Sec. \ref{sec:th_langevin}. 
...
Equation \eqref{eq:th_P_chi} lumps the complex response of the medium to the local electric field into a single, macroscopic parameter $\chi(\br,\omega)$. Such a macroscopic description disregards the microscopic origins of the response (interaction of optical phonons with the electric field \cite{bornhuang}) and does not allow for simultaneous coupling of optical and acoustic phonons as required, e.g., in the description of piezoelectric media \cite{}. % Other problems of the 

These inadequacies are accounted for by a more microscopic model for polarization introduced by Rosa \textit{et al.,} \cite{rosa10,rosa11} for the purpose of quantizing electromagnetic fields in lossy media. They modeled the polarization as a collection of dipoles $\bp_i$ located at points $\br_i$, corresponding to the dipole density
\begin{equation}
 \bb{P}(\br,t) = \sum_i \bp_i(t) \delta(\br-\br_i).
\end{equation}
The dynamics of each dipole was modeled by the classical oscillator model with mass $m_i$ and resonance angular frequency $\omega_0$ so that the equation of motion for the dipole displacement $\bd_i=\bp_i/q$ ($q$ is the dipole charge) becomes Eq. \eqref{eq:th_eom2} \cite{rosa10,rosa11}. The stochastic Langeving force $\xi_i$ and friction $m_i\gamma \dot{\bd}_i$ account for thermal fluctuations and dissipation, respectively, and are equivalent to the corresponding Langevin terms in the phonon equation of motion \eqref{eq:th_eom}. In a fully optics-based treatment, the microscopic parameters $q$, $m_i$, and $\gamma$ do not need to be specified, because they can be absorbed into the definition of the local polarizability in the final results (see Refs. \cite{rosa10,rosa11} and \citepub{dipole}). 

The local electric field $\bE_{ij}$ acting as a force term in Eq. \eqref{eq:th_eom2} and coupling different dipoles can be written in frequency-domain as \cite{novotny,rosa11}
\begin{equation}
...
\tilde \bE_{ij}(\omega) = \omega^2 \mu_0 \mathbb{G}(\br_i,\br_j;\omega) \tilde{\bb{p}}_j(\omega), \label{eq:th_Etilde}
\end{equation}
...
\end{equation}
The three-by-three unit dyadic is denoted by $\unitdyadic$. For generality, we have assumed that the point-like dipole is located in an inhomogeneous environment with dielectric constant $\epsenv(\br,\omega)$ to allow for modeling dipole-dipole energy transfer in a mirror cavity as in \citepub{dipole}. The calculation of Green's dyadics for planar multilayer geometries has been presented by Toma\v{s} \cite{tomas95}. By Fourier transforming Eq. \eqref{eq:th_eom2}, substituting Eq. \eqref{eq:th_Etilde} and rearranging one gets the linear system of equations \eqref{eq:th_eom_fourier_photon} for the dipole displacements.
%\begin{alignat}{2}
...
\end{equation}
where $m_e$ is the electron mass and $V(\br-\bb{R}_i)$ is the (shielded) electrostatic potential from ion $i$. To arrive at the tight-binding approximation, we first solve the energy levels and eigenstates $\phi_i^{\alpha}(\br)$ for a \textit{single} ion:
\begin{equation}
 \left[ - \frac{\hbar^2}{2m_e} \nabla^2  +  V(\br-\bb{R}_i) \right] \phi_i^{\alpha} (\br) = E_i^{\alpha} \phi_i^{\alpha}(\br) \label{eq:th_schrode}
\end{equation}
The bonded solutions of Eq. \eqref{eq:th_schrode} are localized at $\bb{R}_i$ due to the attractive ion potential $V(\br-\bb{R}_i)$. The tight-binding approximation is to assume that the localized orbitals $\phi_i^{\alpha}$ form a complete set \footnote{Typically only a small subset is used, as tightly bound states do not need to be included. More generally, Wannier functions can be used as basis functions \cite{ashcroftmermin}.} so that the electron eigenstates of Hamiltonian \eqref{eq:th_Hel} can be written as
\begin{equation}
 \psi(\br) = \sum_{i,\alpha} c_i^{\alpha} \phi_{i}^{\alpha}(\br).
\end{equation}
Assuming for simplicity that the overlap matrix element $\langle \phi_i^{\alpha}| \phi_j^{\beta} \rangle$ is negligible for $i\neq j$, one can calculate the Hamiltonian matrix elements of the basis functions to be
\begin{equation}
 \langle \phi_i^{\alpha} | \ca{H}_{\textrm{el}} | \phi_j^{\beta} \rangle = E_i^{\alpha} \delta_{ij}\delta_{\alpha\beta}  - t_{ij}^{\alpha\beta}, \label{eq:th_Helements}
\end{equation}
where the ''hopping'' matrix element is 
\begin{equation}
  t_{ij}^{\alpha\beta} = - \int d\bb{r} \phi_i^{\alpha}(\br)^*  \sum_{k\neq j} V(\br-\bb{R}_k)   \phi_j^{\beta}(\br).
\end{equation}
Using Eq. \eqref{eq:th_Helements}, one can write the tight-binding Hamiltonian in the braket notation \cite{schwabl} as 
\begin{equation}
 \ca{H}_{\textrm{el}}^{\textrm{tb}} = \sum_{i,j} \sum_{\alpha,\beta} (E_i^{\alpha} \delta_{ij}\delta_{\alpha\beta}  - t_{ij}^{\alpha\beta})|\phi_i^{\alpha} \rangle \langle \phi_j^{\beta} | \label{eq:th_Hel_tb}
\end{equation}
The equivalent second-quantized form  \cite{schwabl} for Eq. \eqref{eq:th_Hel_tb} is 
\begin{equation}
 \ca{H}_{\textrm{el}}^{\textrm{tb}} = \sum_{i,j} \sum_{\alpha,\beta} (E_i^{\alpha} \delta_{ij}\delta_{\alpha\beta}  - t_{ij}^{\alpha\beta}) c_i^{\alpha\dagger} c_j^{\beta} \label{eq:th_Hel_tb_final}
\end{equation}
where the electron creation operator $c_i^{\alpha\dagger}$ creates an electron in orbital $\phi_i^{\alpha}$ and annihilation operator $c_j^{\beta}$ annihilates electrons at orbital $\phi_j^{\beta}$:
 \begin{subequations}
  \begin{align} 
     c_i^{\alpha\dagger} |0 \rangle = | \phi_i^{\alpha} \rangle,  \\
     c_j^{\beta} |\phi_j^{\beta} \rangle = | 0 \rangle.
  \end{align}
 \end{subequations}
...
The tight-binding Hamiltonian \eqref{eq:th_Hel_tb_final} can also be derived from spatial discretization of the one-particle Schr\"odinger equation \eqref{eq:th_Hel}, in which case the index $i$ labels the discretization grid points \cite{datta}. % The local term $t_{ii}$ in Eq. \eqref{eq:th_Hel} accounts for variations in the local potential created by, e.g., external electric fields \cite{datta}. %For graphene, the hopping constant between nearest-neighbor carbon atoms is approximately $2.5$ eV \cite{dassarma11}.

With the Hamiltonian of Eq. \eqref{eq:th_Hel_tb_final}, the Heisenberg equation of motion $i\hbar \dot{c}_i = - \left[\ca{H}_{\textrm{el}}^{\textrm{tb}},c_i \right]$ \cite{ballentine} accompanied by the stochastic Langevin force $\eta_i$ and Ohmic damping $-i\hbar \gamma_e c_i(t)$ \cite{dhar03} leads to the equation of motion \eqref{eq:th_eom3}. Fourier transform and rearranging gives the linear system of equations \eqref{eq:th_eom_fourier_electron}. 
...
In the classical Langevin theory, the system under study is imagined to interact with an infinite collection of harmonic oscillators. Assuming that the harmonic oscillators were at thermal equilibrium at a prescribed temperature in the infinite past, one can integrate out the bath degrees of freedom so that their interactions with the system are effectively described by fluctuating and dissipative forces \cite{weiss,dhar06}. The fluctuating force is a random variable with zero average and its variance is fixed by the fluctuation-dissipation theorem (FDT) \cite{nyquist28,callen51}, which relates the strength of fluctuations to the dissipation. FDT essentially ensures the onset of statistical thermal equilibrium when all bath temperatures are equal.

%  (and chemical potential in case of electrons)

For Ohmic coupling to a bath of harmonic oscillators, assumed for atomic displacements in Eqs. \eqref{eq:th_eom1} and dipole displacements in Eq. \eqref{eq:th_eom2}, the Langevin force $\xi_i$ is related to the friction parameter $\gamma$ and bath temperature $T_i$ through the FDT \cite{weiss,dhar06}
\begin{equation}
...
\end{equation}
Here the angular brackets stand for statistical ensemble average, the Bose-Einstein function was defined in Eq. \eqref{eq:th_fBE} and the unit matrix $\unitdyadic$ ensures that fluctuations in different coordinate directions are independent. Equation \eqref{eq:th_xixiom_ohmic_qm} was used in the quantum-mechanical Green's function calculations of Publications \cp{gf} and \cp{dipole} for the noise variances. 

In the classical high-temperature limit ($k_B T \gg \hbar \omega$) relevant in the molecular dynamics simulations of Publications \cp{fpu}, \cp{fpu2}, \cp{spectral}, \cp{cnt}, FDT \eqref{eq:th_xixiom_ohmic_qm} reduces to the classical FDT
\begin{equation}
...
If the background field $\Eenvtilde(\br,\omega)$ appearing as a force term in Eq. \eqref{eq:th_eom_fourier_photon} is assumed to be a thermal field arising from the environment with dielectric constant $\epsenv(\br,\omega)$ and temperature $\Tenv$, the field satisfies the FDT \cite{novotny}
\begin{equation}
...
\end{equation}
where the electric Green's dyadic satisfies Eq. \eqref{eq:intro_gemdef}. Equation \eqref{eq:th_e0e0om} was used for the background field in \citepub{dipole}.
...
For the coupling of electrons to an electron bath, the variance of the ''force'' $\eta_i$ appearing as a source term in Eq. \eqref{eq:th_eom2} is \cite{dhar03,dhar06b,roy07}
\begin{equation}
 \langle \tilde \eta_{i}^{\dagger}(\omega) \tilde \eta_{j}(\omega') \rangle = 4\pi\delta(\omega-\omega') \gamma_e f_{\textrm{FD}}(\omega;\mu_{i},T_{i}) \delta_{ij}, \label{eq:th_etaetaom}
\end{equation}
where $\mu_i$ is the bath chemical potential and 
\begin{equation}
 f_{\textrm{FD}}(\omega;T,\mu)=\left\{\exp\left[(\hbar \omega-\mu)/k_BT\right]+1\right\}^{-1}
\end{equation}
is the Fermi-Dirac function. Equation \eqref{eq:th_etaetaom} was applied in Ref. \cite{roy07} to investigate the ballistic-diffusive transition and local heating in one-dimensional chains.
...
This section discusses the energy currents and their calculation in more detail. The most relevant energy currents in the models considered this thesis are (i) the energy currents flowing to local baths or to left and right leads, corresponding to the locally dissipated power or total energy current, respectively, and (ii) interatomic heat current in vibrational heat transfer, corresponding to thermal conduction. Microscopic expressions for these energy currents are derived from local energy balance as shown in Sec. \ref{sec:th_energybalance}.

The thermal average of bath heat currents in linear systems is generally given by the Landauer-B\"uttiker-like expression \eqref{eq:th_QI}, and this derivation is briefly outlined in Sec. \ref{sec:th_bathcurrents}. This expression is used to (i) determine self-consistent temperature profiles and thermal conductances in the self-consistent heat bath model considered in \citepub{gf}, and (ii) calculating the locally absorbed power by dipoles in a microcavity, considered in \citepub{dipole}. The requirement of vanishing heat current to local baths can also be shown to give rise to an intuitive quantum-mechanical definition for local temperature, as discussed in \citepub{gf}.

Section \ref{sec:th_spectral_curr} presents spectral analysis methods for interatomic heat currents, giving a detailed picture of the contributions of different vibrational frequencies to thermal conduction in solids. This spectral analysis method, developed in \citepub{spectral}, is especially useful in non-linear systems, where phonon-phonon interactions redistribute energy between different modes. The analysis relies on calculating statistical cross-correlations between atomic trajectories determined from molecular dynamics method. Spectral analysis was applied for investigating the role of anharmonic effects in interfacial thermal conduction in \citepub{spectral} and determinining frequency-dependent phonon mean free paths in carbon nanotubes in \citepub{cnt}. Molecular dynamics method is presented in Sec. \ref{sec:methods_md}.
...
Let us first outline the calculation of the thermal average for the bath heat current \eqref{eq:th_Qibath}. In this thesis, this average is required in the self-consistent heat bath model and fluctuating dipole model, which rely on linear equations of motion \eqref{eq:th_eom1} and \eqref{eq:th_eom2}, respectively. We therefore assume that the solution is given by Eq. \eqref{eq:th_gf_solution} in terms of the Green's function matrix $\bb{G}(\omega)$. The thermal average of $Q_i^{\textrm{bath}}$ can then be calculated by substituting the Fourier transforms \eqref{eq:th_fourier} of $\bu_i$ and $\xi_i$ evaluated at time $t=0$ to Eq. \eqref{eq:th_Qibath} (thermal average of the current is time-independent). Noting that the Fourier transform of velocity $\dot{u}_i^{\alpha}(t)$ is given by $-i\omega \tilde{u}^{\alpha}_i(\omega)$, this procedure gives (see \citepub{gf})
\begin{alignat}{2}
...
\end{alignat}
At this point, one can use the fluctuation-dissipation theorem \eqref{eq:th_xixiom_ohmic_qm} for correlations of the form $\langle \xi_j^{\beta}(\omega)\tilde \xi_k^{\gamma}(\omega') \rangle$, which eliminates one of the frequency integrals. Substituting Eq. \eqref{eq:th_xixiom_ohmic_qm}, rearranging and using the definition of the local bath coupling function $[\Gamma_i(\omega)]_{jk}^{\alpha\beta}=2m_i\gamma\omega \delta_{\alpha\beta}\delta_{ij} \delta_{ik}$ finally leads to the Landauer-B\"uttiker-like expression \eqref{eq:th_QI}. The detailed derivation is presented in \citepub{gf}.


\subsection{Spectral analysis of interatomic lattice heat current}
\label{sec:th_spectral_curr}
Equation \eqref{eq:th_Qij} gives the total heat current between atoms $i$ and $j$, but does not directly give any insight to the actual energy transfer mechanism. The contributions of different frequencies to vibrational energy transfer can be determined by determining the spectral decomposition of $Q_{i\to j}$ as shown in \citepub{spectral}. Because this analysis is most useful for non-linear systems, we do not assume an analytical solution of the form \eqref{eq:th_gf_solution} to the equations of motion. The expression for the spectral heat current is given in terms of a correlation function that can be determined from MD simulations.

The spectral decomposition of interparticle heat current $Q_{i\to j}$ is achieved by substituting the Fourier transforms \eqref{eq:th_fourier} of $\bu_i$ evaluated at time $t=0$ in Eq. \eqref{eq:th_Qij}. One then gets
\begin{alignat}{2}
