...
First-principles modeling of electron transport starts from writing down the quantum-mechanical Hamiltonian $\ca{H}_{\textrm{el}}$ for electrons and ions constituting the atomic lattice \cite{ashcroftmermin}. For the simplicity of discussion, we exclude electron-electron, electron-phonon, and electron-photon interactions and assume that electron dynamics is described by the tight-binding model \cite{wallace47}. The tight-binding Hamiltonian for electrons can then be written as \cite{datta}
\begin{equation}
 \ca{H}_{\textrm{el}} = - \sum_{i,j} c_i^{\dagger} t_{ij} c_j. \label{eq:th_Hel}
\end{equation}
Here, electron creation operator $c_i^{\dagger}$ creates and annihilation operator $c_j$ annihilates electrons at electron orbitals localized at sites $i$ and $j$, respectively. The hopping constant $t_{ij}$ allows for electron hopping between orbitals at atoms $i$ and $j$ and can be calculated from the Hamiltonian matrix element between the orbitals as \cite{}
\begin{equation}
 t_{ij} = - \langle \psi_i | \ca{H} | \psi_j \rangle
\end{equation}
The Hamiltonian \eqref{eq:th_Hel} can also be derived from spatial discretization of the one-particle Schr\"odinger equation \cite{datta}, in which case the index $i$ labels the discretization grid points. The local term $t_{ii}$ in Eq. \eqref{eq:th_Hel} accounts for variations in the local potential created by, e.g., external electric fields \cite{datta}. %For graphene, the hopping constant between nearest-neighbor carbon atoms is approximately $2.5$ eV \cite{dassarma11}.

With the Hamiltonian of Eq. \eqref{eq:th_Hel}, the Heisenberg equation of motion $i\hbar \dot{c}_i = - \left[\ca{H}_{\textrm{el}},c_i \right]$ \cite{ballentine} accompanied by the stochastic Langevin force $\eta_i$ and Ohmic damping $-i\hbar \gamma_e c_i(t)$ \cite{dhar03} leads to the equation of motion \eqref{eq:th_eom3}. Fourier transform and rearranging gives the linear system of equations \eqref{eq:th_eom_fourier_electron}. 
