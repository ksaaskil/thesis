...
\end{equation}
where $\mu_0$ is the vacuum permeability. The Green's dyadic $\mathbb{G}^{\alpha\beta}(\br_i,\br_j;\omega)$ is defined in Sec. \ref{sec:th_eom2_photon}. % In Eq. \eqref{eq:th_eom_fourier_photon}, the electromagnetic Green's dyadic $\mathbb{G}^{\alpha\beta}(\br_i,\br_j;\omega)$ \cite{novotny} describes the electromagnetic coupling of dipoles and is defined in \ref{sec:th_eom2_photon}. 
...
\end{tabularx}
\end{center}
\caption{Comparison of different methods for modeling phononic thermal conduction. \textbf{POSSIBLY REMOVED UNLESS A BETTER FIGURE COULD BE DRAWN.}}
\end{table}
\fi
