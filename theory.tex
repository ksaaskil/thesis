...
Equations of motion for the three degrees of freedom $\bu_i$, $\bb{d}_i$, and $c_i^{\sigma}$ are all connected through the Langevin physics describing fluctuations and dissipation, leading to a similar solution methods and phenomenology despite involving different energy carriers. Formally, the equations are of the form
\begin{equation}
 \left\{ \begin{array}{c}
          \textrm{Rate of} \\
	  \textrm{change} 
...
\begin{align}
 ...
 m_i \ddot{\bb{d}}_i(t) &= - \bb{K}_{ii} \bb{d}_i(t) +q\sum_{j} \bb{E}_{ij}(t)+\xi_i(t)+ q\Eenv(\br_i,t) - m_i \gamma \dot{\bd}_i(t), \label{eq:th_eom2}\\
...
The detailed parameters of Eqs. \eqref{eq:th_eom1}, \eqref{eq:th_eom2} and \eqref{eq:th_eom3} are presented in Sec. \ref{sec:th_eom2}. It is worth noting that whereas equations \eqref{eq:th_eom1} and \eqref{eq:th_eom2} are purely real and possess two time-derivatives in the left-hand side, Eq. \eqref{eq:th_eom3} for the electron annihilation operator $c_i^{\sigma}$ contains the imaginary unit $i$ and contains only a single time-derivative. This difference is rooted in the fundamental property of quantum mechanics that time evolution is modeled by diffusion equation in imaginary time (Schr\"odinger equation), allowing for wave-like solutions in real time \cite{ballentine}.
...
\begin{equation}
 \tilde{\bE}_{ij}^{\alpha}(\omega) = q\omega^2 \mu_0 \sum_{\beta} \mathbb{G}^{\alpha\beta}(\br_i,\br_j;\omega)\tilde{d}_j^{\beta}, 
\end{equation}
...
\begin{equation}
 f_{\textrm{BE}}(\omega,T)=\left[\exp(\hbar \omega/k_BT)-1\right]^{-1}, \label{eq:th_fBE}
\end{equation}
and and the unit matrix $\unitdyadic$ reflects the independence of fluctuations in different coordinate directions. Equation \eqref{eq:th_xixiom_ohmic_qm} was used in the quantum-mechanical Green's function calculations of Publications \cp{gf} and \cp{dipole} for the noise variances. 
...
Let us outline the calculation of the thermal average for the bath heat current \eqref{eq:th_Qibath} in phonon thermal conduction. The solution for the atomic displacements $\tilde{\bu}_i(\omega)$ at each site $i$ is given in terms of the Green's function matrix $\bb{G}(\omega)$ as
\begin{equation}
...
\end{alignat}
At this point, one can use the fluctuation-dissipation theorem \eqref{eq:th_xixiom_ohmic_qm} for correlations of the form $\langle \xi_j^{\beta}(\omega)\tilde \xi_k^{\gamma}(\omega') \rangle$, which eliminates one of the frequency integrals. Substituting Eq. \eqref{eq:th_xixiom_ohmic_qm}, rearranging and using the definition of the local bath coupling function $[\Gamma^i(\omega)]_{jk}^{\alpha\beta}=2m_i\gamma\omega \delta_{\alpha\beta}\delta_{ij} \delta_{ik}$, one finally gets the Landauer-B\"uttiker-like \cite{landauer57,buttiker92} expression (see \citepub{gf})
\begin{equation}
  Q_i^{\textrm{bath}} = \int_0^{\infty} \frac{d\omega}{2\pi} \hbar \omega  \sum_J \ca{T}_{ij}(\omega) \left[ f_{\textrm{BE}}(\omega,T_j)- f_{\textrm{BE}}(\omega,T_i)\right]. \label{eq:th_QI}
\end{equation}
...
Equation \eqref{eq:th_Qij} gives the total phononic heat current between atoms $i$ and $j$, but does not directly give any insight to the actual energy transfer mechanism. The contributions of different frequencies to vibrational energy transfer can be determined by determining the spectral decomposition of $Q_{i\to j}$ as shown below and in detail in \citepub{spectral}. Because this analysis is most useful for non-linear systems, we do not assume an analytical solution of the form \eqref{eq:th_gf_solution} to the equations of motion. The expression for the spectral heat current is given in terms of a correlation function that can be determined from MD simulations.

The spectral decomposition of interparticle heat current $Q_{i\to j}$ is found  by substituting the Fourier transforms \eqref{eq:th_fourier} of $\bu_i$ evaluated at time $t=0$ in Eq. \eqref{eq:th_Qij}. One then gets
\begin{alignat}{2}
...
Including anharmonic terms in the local energy \eqref{eq:th_ei} gives rise to additional terms in the spectral heat current, corresponding to higher-order processes such as three-phonon energy transfer. The spectral analysis of these higher-order terms is straightforward but more complicated. Details are given in \citepub{spectral}, where the contributions of three-phonon processes to interfacial energy transfer were investigated. 
